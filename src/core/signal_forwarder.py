...
class SignalForwarder:
    ...
    async def process_message(self, message: str, metadata: Dict[str, Any]) -> bool:
        ...
        # Telegram gibt manchmal positive IDs zurück für Supergroups
        ...
        # Nur Nachrichten aus der überwachten VIP-Gruppe verarbeiten
        if normalized_chat_id != -1002299206473:
            return False
        ...
        try:
            # Für Trading-Updates: Direkt weiterleiten ohne komplexes Parsing
            if self._is_trading_update(message):
                # Einfach weiterleiten
                await self.send_telegram_callback(self.target_group_id, message)
                
                # Trading update weitergeleitet
                
                return True
            
            # Für vollständige Signale: Parsen und formatieren
    ...
    def _is_signal(self, message: str) -> bool:
        ...
        # Trading-Updates und Status-Nachrichten (auch weiterleiten)
        trading_update_indicators = [
            ...
            # Neue Signal-Ankündigungen
            ("/USDT" in message and any(kw in message.upper() for kw in ["LONG", "SHORT", "BUY", "SELL"])),
            # Kryptowährungspaare allgemein
            bool(re.search(r'[A-Z0-9]{2,8}/USDT', message))
        ]
        
        # Signal ist gültig wenn es entweder ein vollständiges Signal ODER ein Trading-Update ist
        is_full_signal = all(full_signal_indicators)
        is_trading_update = any(trading_update_indicators)
        
        return is_full_signal or is_trading_update
    
    def _is_trading_update(self, message: str) -> bool:
        """Prüft ob es sich um ein Trading-Update handelt (nicht um ein vollständiges Signal)."""
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "monitored_chat_id": -1002299206473,
