    ...
    try:
        ...
        try:
            ...
            if hasattr(entity, 'broadcast') and entity.broadcast:
                # It's a channel
                formatted_id = "-100{}".format(chat_id)
            else:
                # It's a group or other
                formatted_id = "-{}".format(chat_id) if chat_id > 0 else str(chat_id)
            
            print("✅ Found @{}!".format(channel_username))
            print("📋 Details:")
            print("   Title: {}".format(getattr(entity, 'title', 'N/A')))
            print("   Raw ID: {}".format(chat_id))
            print("   Formatted ID: {}".format(formatted_id))
            print("   Type: {}".format('Channel' if hasattr(entity, 'broadcast') and entity.broadcast else 'Group'))
            print()
            print("🎯 Use this ID in .env:")
            print("   MONITORED_CHAT_IDS=-2299206473,{}".format(formatted_id))
            
            return formatted_id
            
        except Exception as e:
            print("❌ Could not find @{}: {}".format(channel_username, e))
            print()
