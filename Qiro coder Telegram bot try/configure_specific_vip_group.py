...
class SpecificVIPGroupTester:
    ...
    async def test_vip_group_access(self) -> Dict[str, Any]:
        ...
        print(f"\n🎯 TESTE SPEZIFISCHE VIP-GRUPPE: {self.target_vip_group_id}")
        print("=" * 50)
            ...
            async for message in self.client.iter_messages(entity, limit=15):
                ...
                if message.text and message.text.strip():
                    ...
                    if re.search(r'[A-Z]{2,8}/USDT', message.text):
                        ...
                        print(f"📊 Signal gefunden: ID {message.id} - {message.date.strftime('%H:%M:%S') if message.date else 'Unknown'}")
            
            print(f"✅ Nachrichten-Zugriff erfolgreich!")
            print(f"   Total: {message_count} Nachrichten")
            print(f"   Mit Text: {len(messages)} Nachrichten")
            print(f"   Trading-Signale: {len(signals)} gefunden")
    ...
    async def test_cryptet_access(self) -> Dict[str, Any]:
        ...
        print(f"\n🔍 TESTE CRYPTET-KANAL: {self.cryptet_channel_id}")
        print("=" * 40)
